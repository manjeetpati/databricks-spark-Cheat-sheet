...
# As you are aware PySpark is designed to process large datasets with 100x faster than the tradition processing, this wouldnâ€™t have been possible with out partition. Below are some of the advantages using PySpark partitions on memory or on disk.
# 
...
# 
# 3. Create DataFrame
# Letâ€™s Create a DataFrame by reading a CSV file. You can find the dataset explained in this article at Github zipcodes.csv file
...
# 
# pyspark partitionby
# partitionBy(â€œstateâ€) example output
...
# partitonBy(â€œstateâ€,â€cityâ€) multiple columns
...
# 
# 7. Data Skew â€“ Control Number of Records per Partition File
...
# 
# 9. PySpark SQL â€“ Read Partition Data
...
# 
# Letâ€™s assume you have a US census table that contains zip code, city, state, and other columns. Creating a partition on the state, splits the table into around 50 partitions, when searching for a zipcode within a state (state=â€™CAâ€™ and zipCode =â€™92704â€²) results in faster as it needs to scan only in a state=CA partition directory.
# 
